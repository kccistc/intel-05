...
# Arduino와의 직렬 통신 설정 (포트와 속도 확인)
# Arduino와 시리얼 포트 연결 설정
...
def draw_poses(img, poses, point_score_threshold, skeleton=default_skeleton):
    ...
    for pose in poses:
        ...
        # 손목, 눈, 어깨 좌표
        left_wrist = points[10]
        right_wrist = points[9]
        left_eye = points[2]
        right_eye = points[1]
        left_shoulder = points[6]
        right_shoulder = points[5]

        # 손목과 눈의 y좌표와 x좌표 비교
        right_hand_near_eye = False
        left_hand_near_eye = False

        # 오른쪽 손목이 오른쪽 눈의 y좌표와 x좌표가 비슷한지 확인
        if points_scores[9] > point_score_threshold and points_scores[1] > point_score_threshold:
            if abs(right_wrist[1] - right_eye[1]) <= 50 and abs(right_wrist[0] - right_eye[0]) <= 200:
                right_hand_near_eye = True

        # 왼쪽 손목이 왼쪽 눈의 y좌표와 x좌표가 비슷한지 확인
        if points_scores[10] > point_score_threshold and points_scores[2] > point_score_threshold:
            if abs(left_wrist[1] - left_eye[1]) <= 50 and abs(left_wrist[0] - left_eye[0]) <= 200:
                left_hand_near_eye = True

        # 양손 모두 조건을 만족하면 'slowly' 출력
        if right_hand_near_eye and left_hand_near_eye:
            cv2.putText(img, "slowly", (20, 100), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 255, 255), 2)
            send_command_to_arduino("slowly")
        # 양손이 아닌 경우, 각각 'stop' 출력
        elif right_hand_near_eye or left_hand_near_eye:
            cv2.putText(img, "stop", (20, 140), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
            send_command_to_arduino("stop")
            
        # 오른손이 왼쪽 어깨와 x좌표가 비슷해지면 'go left' 출력
        elif points_scores[9] > point_score_threshold and points_scores[5] > point_score_threshold:
            if abs(right_wrist[0] - left_shoulder[0]) <= 60:
                cv2.putText(img, "go left", (20, 140), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 0, 0), 2)
                send_command_to_arduino("left")

        # 왼손이 오른쪽 어깨와 x좌표가 비슷해지면 'go right' 출력
        elif points_scores[10] > point_score_threshold and points_scores[6] > point_score_threshold:
            if abs(left_wrist[0] - right_shoulder[0]) <= 60:
                cv2.putText(img, "go right", (20, 180), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 2)
                send_command_to_arduino("right")
        else: send_command_to_arduino("go")
